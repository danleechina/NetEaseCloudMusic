...
#import "NetEaseCloudMusic-Swift.h"
...
CGFloat SELECTOR_HEIGHT = 3.0; //%%% thickness of the selector bar
...
#define DefaultRedColor [UIColor colorWithRed:183/255.0 green:39/255.0 blue:18/255.0 alpha:1]
...
@property (nonatomic, strong) NSMutableArray<UIButton *>* navigationButtons;
...
- (void)viewDidLoad
{
    ...
    SongSheetViewController *vc2 = [SongSheetViewController new];
    RadioViewController *vc3 = [RadioViewController new];
    RankListViewController *vc4 = [RankListViewController new];
    [viewControllerArray addObjectsFromArray:@[vc1, vc2, vc3, vc4]];
...
-(void)setupSegmentButtons {
    ...
    if (!buttonText) {
         buttonText = [[NSArray alloc]initWithObjects: @"个性推荐", @"歌单", @"主播电台", @"排行榜", nil]; //%%%buttontitle
    }
    
    self.navigationButtons = [NSMutableArray new];
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 1, self.view.frame.size.width, 1)];
    lineView.backgroundColor = [UIColor lightGrayColor];
    [navigationView addSubview:lineView];
    
    [pageController.view addSubview:navigationView];
    [self.navigationButtons[self.currentPageIndex] setTitleColor:DefaultRedColor forState:UIControlStateNormal];
